...
\node[crypto, below=3.0cm of fernet] (dcif)    {\texttt{descifrar\_embedding()}};
\node[box,    left=of dcif]          (compare) {Comparación\\\scriptsize{DeepFace.verify}};

% --- Fallback ---
\node[fallback, below=1.0cm of dbn, xshift=2.0cm] (legacy) {Fallback legacy\\\scriptsize{embedding sin cifrar}};
...
% --- Camino fallback ---
\draw[darrow] (legacy.south) |- ([xshift=0.6cm]dcif.east)
              node[pos=0.25, right, font=\scriptsize] {parse directo};

% --- Etiquetas ---
\node[lbl, above=0.1cm of cif] {clave Fernet (\texttt{.env})};
\node[lbl, above=0.6cm of dcif] {misma clave (rotable)};

\end{tikzpicture}%
}
\caption[Pipeline de cifrado biométrico]{Pipeline de cifrado biométrico con Fernet (AES-\allowbreak 128-\allowbreak CBC + HMAC-\allowbreak SHA256), con fallback para embeddings legacy almacenados sin cifrar.}
